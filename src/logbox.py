...
class LogBox(GFXEntity):
    ...
    def __init__(self, pos: Tuple[int, int], size: Tuple[int, int], color: Tuple[int, int, int],
        ...
        x, y = pos
        self._pos = [(x + self.text_x, y + self.text_y + j * self.text_gap) for j in range(self._n_texts)]
        self._rects = [pygame.Rect(rect_pos, self.text_rect_size) for rect_pos in self._pos]
